...
$.fn.dataTable.ext.search.push(
    function( settings, data, dataIndex ) {
        ...
        let filterStatus = $('#filter-stock').val();
...
$.fn.dataTable.ext.search.push(
    function( settings, data, dataIndex ) {
        let filterStatus = $('#filter-kasir-status').val();
        let filterStock = $('#filter-kasir-stock').val();
        
        if(filterStatus || filterStock) {
            if(filterStatus == data[7] || filterStock == data[8]) {
                return true;
            }
        } else {
            return true;
...
$(document).ready(function() {
    ...
    var tableInvoice = $('#simpletable-invoice').DataTable();
    
    
    var tableKasir = $('#simpletable-kasir').DataTable();

    $('#filter-kasir-stock, #filter-kasir-status').change(function(){
  
        tableKasir.draw();
    })
    
    $('#simpletable-kasir').on('click', '.btn-paket-publish', function(e){
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function( response ) {
                ...
                btn.data('publish', (publish == 'Draft' ? 'Publish' : 'Draft'))
                btn.html((publish == 'Draft') ? `<span class='ik ik-x'></span>` : `<span class='ik ik-check'></span>`); 
                btn.parent().parent().prev().prev().html( publish );
